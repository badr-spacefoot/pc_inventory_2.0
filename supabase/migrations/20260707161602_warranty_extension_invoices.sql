...
update public.device_invoices
set invoice_type = 'purchase'
where invoice_type is null or invoice_type = '';

alter table public.device_invoices
  alter column invoice_type set default 'purchase',
  alter column invoice_type set not null;

alter table public.device_invoices
  drop constraint if exists device_invoices_invoice_type_check,
  add constraint device_invoices_invoice_type_check
    check (invoice_type in ('purchase', 'warranty_extension', 'repair', 'accessory', 'other'));
...
alter table public.device_invoices
  drop constraint if exists device_invoices_warranty_duration_check,
  add constraint device_invoices_warranty_duration_check
    check (warranty_duration_months is null or warranty_duration_months >= 0);

create index if not exists idx_device_invoices_device_type_date
  on public.device_invoices(device_id, invoice_type, invoice_date desc nulls last, created_at desc);

create index if not exists idx_device_invoices_warranty_end
  on public.device_invoices(device_id, warranty_end_date desc nulls last)
  where invoice_type = 'warranty_extension';
