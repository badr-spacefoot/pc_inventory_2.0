...
async function handleProfile(request: Request) {
  ...
  if (!team && !proposedTeam) return badRequest(request, "Équipe requise ou proposition d'équipe requise.");
  if (!establishment && !proposedEstablishment) return badRequest(request, "Établissement requis ou proposition d'établissement requise.");
  ...
  if (!accessTokenId) return badRequest(request, "Token de collecte invalide, expiré, révoqué ou épuisé.", 401);
...
async function handleGetCollectionPrefill(request: Request, code: string) {
  ...
  if (!prefillCode) return badRequest(request, "Code de pré-remplissage requis.", 400);
  const { data, error } = await supabase
  ...
  if (!data || new Date(safeString(data.expires_at)).getTime() <= Date.now()) {
    return badRequest(request, "Code de pré-remplissage invalide ou expiré.", 404);
  }
...
async function handleAdminSaveUser(request: Request, id?: string) {
  ...
  if (!id && !password) return badRequest(request, "Mot de passe requis pour créer un utilisateur.");
  ...
  const { data, error } = await query
    ...
    .single();
  if (error) {
    if (error.code === "23505") return badRequest(request, "Un compte utilise déjà cet identifiant ou email.", 409);
    throw error;
  }
...
async function handleAdminCreateAccessToken(request: Request) {
  ...
  if (!Number.isFinite(durationHours)) return badRequest(request, "Durée invalide.");
...
async function handleAdminImportLegacyHistory(request: Request) {
  ...
  if (rows.length === 0) return badRequest(request, "Aucune ligne historique à importer.");
  if (rows.length > 1000) return badRequest(request, "Import limité à 1000 lignes par appel.");
...
async function handleAdminSaveTeam(request: Request, id?: string) {
  ...
  if (error) {
    if (error.code === "23505") return badRequest(request, "Une équipe porte déjà ce nom.", 409);
    throw error;
  }
...
async function handleAdminSaveEstablishment(request: Request, id?: string) {
  ...
  if (!allowedTypes.includes(establishmentType)) return badRequest(request, "Type d'établissement invalide.");
  ...
  const { data, error } = await query
    ...
    .single();
  if (error) {
    if (error.code === "23505") return badRequest(request, "Un établissement porte déjà ce nom.", 409);
    throw error;
  }
...
async function handleAdminDeviceAssignment(request: Request, id: string) {
  ...
  if (!teamValid) return badRequest(request, "?quipe s?lectionn?e introuvable.", 404);
  if (!establishmentValid) return badRequest(request, "?tablissement s?lectionn? introuvable.", 404);
  if (!userValid) return badRequest(request, "Utilisateur s?lectionn? introuvable.", 404);
  ...
  if (ownerChanged) {
    if (!ownerEmail && !assignedUserId) return badRequest(request, "Email propriétaire requis pour créer ou modifier un propriétaire.");
    if (assignedUserId) {
      if (ownerEmail) {
        ...
        if (duplicateUser) return badRequest(request, "Un autre utilisateur utilise d?j? cet email.", 409);
      }
