...
alter table public.hardware_enrichment add column if not exists cpu_release_catalog_id uuid;
...
create table if not exists public.cpu_release_catalog (
  ...
  release_precision text not null default 'unknown' check (release_precision in ('day', 'month', 'quarter', 'half_year', 'year', 'unknown')),
  release_event_type text not null default 'unknown' check (release_event_type in ('announcement', 'launch', 'first_product_availability', 'expected_availability', 'unknown')),
  ...
  match_scope text not null default 'exact_name' check (match_scope in ('part_number', 'exact_name', 'alias', 'family')),
  content_hash text,
  etag text,
  last_modified text,
...
create table if not exists public.cpu_release_aliases (
  ...
  alias_type text not null default 'collector' check (alias_type in ('collector', 'manufacturer', 'benchmark', 'part_number', 'manual')),
  created_at timestamptz not null default now(),
...
alter table public.hardware_enrichment
  drop constraint if exists hardware_enrichment_cpu_release_catalog_id_fkey;
alter table public.hardware_enrichment
  add constraint hardware_enrichment_cpu_release_catalog_id_fkey
  foreign key (cpu_release_catalog_id) references public.cpu_release_catalog(id) on delete set null;
alter table public.hardware_enrichment
...
create unique index if not exists uq_cpu_release_catalog_vendor_name on public.cpu_release_catalog(vendor, normalized_name);
create unique index if not exists uq_cpu_release_catalog_vendor_part on public.cpu_release_catalog(vendor, lower(part_number)) where part_number is not null and btrim(part_number) <> '';
create index if not exists idx_cpu_release_catalog_verified on public.cpu_release_catalog(last_verified_at);
create index if not exists idx_cpu_release_catalog_precision on public.cpu_release_catalog(release_precision);
create index if not exists idx_cpu_release_catalog_vendor_family on public.cpu_release_catalog(vendor, family);
create index if not exists idx_cpu_release_aliases_normalized on public.cpu_release_aliases(normalized_alias);
create index if not exists idx_cpu_release_sync_runs_vendor_started on public.cpu_release_sync_runs(vendor, started_at desc);
create index if not exists idx_hardware_enrichment_cpu_release_catalog on public.hardware_enrichment(cpu_release_catalog_id);
